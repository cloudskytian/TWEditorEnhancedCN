...
public class StringsDatabase {
	...
	public StringsDatabase(String filePath) throws DBException, IOException {
		this(new File(filePath));
	}

	public StringsDatabase(File file) throws DBException, IOException {
		this.file = file;
		this.in = new RandomAccessFile(file, "r");
		readHeader();
	}

	private void readHeader() throws DBException, IOException {
		byte[] buffer = new byte[20];
		int count = this.in.read(buffer);
		if (count != buffer.length) {
			throw new DBException("TLK header被截断");
		}
		String type = new String(buffer, 0, 4);
		String version = new String(buffer, 4, 4);
		if (!type.equals("TLK ")) {
			throw new DBException(new StringBuilder().append("文件类型'").append(type).append("'不支持").toString());
		}
		if (!version.equals("V3.0")) {
			throw new DBException(new StringBuilder().append("文件版本'").append(version).append("'不支持").toString());
		}
	...
	public String getString(int stringRef) {
		...
		try {
			...
			if (refid < this.stringCount) {
				...
				int count = this.in.read(buffer);
				if (count != buffer.length) {
					throw new DBException(new StringBuilder().append("字符串输入被截断以供参考").append(refid).toString());
				}
				if ((buffer[0] & 0x1) != 0) {
					...
					if (count != length) {
						throw new DBException(new StringBuilder().append("字符串数据被截断以供参考").append(refid).toString());
					}
					string = new String(data, "UTF-8");
				}
			}
		} catch (DBException exc) {
			Main.logException("字符串数据库格式错误", exc);
		} catch (IOException exc) {
			Main.logException("无法读取字符串数据库", exc);
		}

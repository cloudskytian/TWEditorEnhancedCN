...
public class Quest {
	...
	public Quest(String resource, DBElement element) throws DBException {
		this.resourceName = resource;
		this.questElement = element;
		if (element.getType() != 14) {
			throw new DBException("顶级任务元素结构错误");
		}
		...
		if ((mainPhase == null) || (mainPhase.getType() != 15)) {
			throw new DBException("任务MainPhase找不到" + this.resourceName);
		}
		DBList questList = (DBList) mainPhase.getValue();
		if (questList.getElementCount() == 0) {
			throw new DBException("没有任务列表" + this.resourceName);
		}

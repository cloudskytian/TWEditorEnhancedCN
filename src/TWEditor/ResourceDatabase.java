...
public class ResourceDatabase {
	...
	public void load() throws DBException, IOException {
		...
		try {
			...
			if (count != 160) {
				throw new DBException("Database header太小");
			}
			...
			if (!validType) {
				throw new DBException("数据库类型'" + this.databaseType + "'不支持");
			}
			...
			if (!validVersion) {
				throw new DBException("数据库版本'" + this.databaseVersion + "'不支持");
			}
			...
			if (stringCount > 0) {
				...
				for (int i = 0; i < stringCount; i++) {
					...
					if (count != 8) {
						throw new DBException("字符串列表被截断");
					}
			...
			if (entryCount > 0) {
				...
				for (int i = 0; i < entryCount; i++) {
					count = in.read(key);
					if (count != keyLength) {
						throw new DBException("Key列表被截断");
					}
					for (count = 0; (count < nameLength) && (key[count] != 0); count++)
						;
		...
		try {
			...
			for (ResourceEntry entry : this.entries) {
				...
				if (nameBytes.length > nameLength) {
					throw new DBException("资源名称'" + entry.getResourceName() + "'太长");
				}
				int index;
			...
			for (ResourceEntry entry : this.entries) {
				...
				while (residualLength > 0) {
					...
					if (count != length) {
						throw new DBException("资源" + entry.getName() + "的数据被截断");
					}
			...
			if (!outputFile.renameTo(this.file)) {
				throw new IOException("无法重命名" + outputFile.getName() + "为" + this.file.getName());
			}
		} finally {
			if (in != null) {
				in.close();
	...
	public void setType(String type) {
		...
		if (!validType) {
			throw new IllegalArgumentException("数据库类型'" + type + "'不支持");
		}
	...
	public void setVersion(String version) {
		...
		if (!validVersion) {
			throw new IllegalArgumentException("数据库版本'" + version + "'不支持");
		}

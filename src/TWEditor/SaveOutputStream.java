...
public class SaveOutputStream extends OutputStream {
	...
	public SaveOutputStream(SaveEntry entry) throws IOException {
		this.entry = entry;
	...
	public void write(byte[] buffer, int bufferOffset, int bufferLength) throws IOException {
		if (this.entry == null) {
			throw new IOException("输出流未打开");
		}
		if (this.outputStream != null) {
	...
	protected void finalize() {
		try {
			...
			Main.logException("最终确定输出流时出现异常", exc);
		}
	}
}
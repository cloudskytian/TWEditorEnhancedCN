...
public class QuestsPanel extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent ae) {
		try {
			...
			if (action.equals("examine started")) {
				...
				if (sel < 0) {
					JOptionPane.showMessageDialog(this, "你必须选择要查看的任务", "未选择任务", 0);
				} else
	...
	private void examineQuest(Quest quest) throws DBException {
		...
		if ((element == null) || (element.getType() != 15)) {
			throw new DBException(new StringBuilder().append("找不到任务的阶段列表").append(quest.getResourceName()).toString());
		}
